...
async function deleteKeyword(id) {
  try {
    // Corrected URL format (using path parameter instead of query string)
    const response = await fetch(`${API_URL}/${id}`, { method: "DELETE" });  // Use `/id` instead of `?id=id`
    if (!response.ok) throw new Error(`HTTP Error: ${response.status}`);
    fetchKeywords();  // Refresh the keyword list after deletion
  } catch (err) {
